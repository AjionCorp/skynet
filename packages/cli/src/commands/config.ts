...
const KNOWN_VARS: Record<string, string> = {
  ...
  SKYNET_CLAUDE_MODEL: "Claude model override",
  SKYNET_AGENT_PLUGIN: "Agent plugin (auto|claude|codex|gemini|path)",
  SKYNET_CODEX_BIN: "Codex binary path",
  SKYNET_CODEX_SUBCOMMAND: "Codex subcommand to run (exec recommended for non-interactive)",
  SKYNET_CODEX_MODEL: "Codex model override",
  SKYNET_CODEX_FLAGS: "Codex CLI flags",
  SKYNET_GEMINI_BIN: "Gemini binary path",
  SKYNET_GEMINI_FLAGS: "Gemini CLI flags",
  SKYNET_GEMINI_MODEL: "Gemini model override",
  SKYNET_GEMINI_NOTIFY_INTERVAL: "Gemini auth notify interval (seconds)",
  SKYNET_EXTRA_PATH: "Additional PATH entries",
  SKYNET_ERROR_ENV_KEYS: "Env vars to scan in server logs",
  SKYNET_DASHBOARD_API_KEY: "API key for dashboard auth (empty = auth disabled)",
