...
vi.mock("@ajioncorp/skynet/handlers", () => ({
  createPipelineExplainHandler: vi.fn(() => mockGET),
}));
...
describe("GET /api/admin/pipeline/explain", () => {
  beforeEach(() => {
    mockGET.mockClear();
  });
  ...
  it("wires createPipelineExplainHandler with config at module load", () => {
    expect(createPipelineExplainHandler).toHaveBeenCalledWith(
  ...
  it("passes request through to the handler", async () => {
    const req = new Request("http://localhost/api/admin/pipeline/explain");
    await GET(req);
