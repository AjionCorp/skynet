...
const BASE_URL = "http://localhost/api/admin/mission/raw";

describe("mission/raw route wiring", () => {
...
describe("GET /api/admin/mission/raw", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("delegates to handler and returns raw content", async () => {
    const res = await GET();
    expect(mockGET).toHaveBeenCalledTimes(1);
    const body = await res.json();
    expect(body.error).toBeNull();
    expect(body.data.raw).toContain("# Mission Raw");
  });
});

describe("PUT /api/admin/mission/raw", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("delegates to handler with request", async () => {
    const req = new Request(BASE_URL, {
      ...
      body: JSON.stringify({ raw: "# Updated Raw" }),
    });
